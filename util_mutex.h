...
APR_DECLARE_OPTIONAL_FN(apr_status_t,ap_mutex_register,(
                                               apr_pool_t *pconf,
                                               const char *type,
 ...
 */
APR_DECLARE_OPTIONAL_FN(apr_status_t,ap_global_mutex_create,(
 ...
 * (currently none are defined for this function)
 */
APR_DECLARE_OPTIONAL_FN(apr_status_t,ap_proc_mutex_create,(
...
#ifndef AP_MUTEX_EXPORT

static apr_status_t ap_mutex_register(apr_pool_t *pconf, const char *type,
                                      const char *default_dir,
                                      apr_lockmech_e default_mech,
                                      apr_int32_t options)
{
    static APR_OPTIONAL_FN_TYPE(ap_mutex_register) *ap_mutex_register_ptr
            = NULL;
    if (!ap_mutex_register_ptr)
        ap_mutex_register_ptr = APR_RETRIEVE_OPTIONAL_FN(ap_mutex_register);
    if (!ap_mutex_register_ptr)
        return APR_ENOTIMPL;
    return ap_mutex_register_ptr(pconf, type, default_dir, default_mech,
                                 options);
}

apr_status_t ap_global_mutex_create(apr_global_mutex_t **mutex,
                                    const char **name, const char *type,
                                    const char *instance_id, server_rec *server,
                                    apr_pool_t *pool, apr_int32_t options)
{
    static APR_OPTIONAL_FN_TYPE(ap_global_mutex_create) *mutex_create = NULL;
    if (!mutex_create)
       mutex_create = APR_RETRIEVE_OPTIONAL_FN(ap_global_mutex_create);
    if (!mutex_create)
        return APR_ENOTIMPL;
    return mutex_create(mutex, name, type, instance_id, server, pool, options);
}
...
apr_status_t ap_proc_mutex_create(apr_proc_mutex_t **mutex,
                                  ...
                                  apr_pool_t *pool, apr_int32_t options)
{
    static APR_OPTIONAL_FN_TYPE(ap_proc_mutex_create) *mutex_create = NULL;
    if (!mutex_create)
       mutex_create = APR_RETRIEVE_OPTIONAL_FN(ap_proc_mutex_create);
    if (!mutex_create)
        return APR_ENOTIMPL;
    return mutex_create(mutex, name, type, instance_id, server, pool, options);
}

#endif /* !AP_MUTEX_EXPORT */
